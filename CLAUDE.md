...
| `scripts/daily_check.py` | 每日資料核對 + Telegram 通知 |
...
2. `update-data.yml` → 跑 `mis_fetcher.py` → 合併 _base.json → 產出 `data/market.json`
...
| 代號 | 說明 |
|---|---|
| D-1 | 財務試算（資金 + 定期定額 + 報酬率 → 複利試算 + ETF 建議） |
| D-2 | ETF 持倉健檢（輸入代碼 → 訊號 / 現價 / 殖利率 / 溢價） |
